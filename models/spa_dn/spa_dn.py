...
class SPA_DN(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        if self.num_feature_levels > len(srcs) or False:
            _len_srcs = len(srcs)
        ...
        img_ids = [target['image_id'] for target in targets] if targets else []

        hs, reference, hs_enc, ref_enc, init_box_proposal = self.transformer(srcs, masks, input_query_bbox, poss,input_query_label,attn_mask,img_ids)
...
@MODULE_BUILD_FUNCS.registe_with_name(module_name='spa_dn')
def build_spa_dn(args):
