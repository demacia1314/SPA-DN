...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
        ...
        self.se_layer = SElayer(d_model, reduction=4)
    ...
    def prepare_for_encoder(self, srcs, masks, pos_embeds):
        src_flatten = []
        mask_flatten = []
    ...
    def prepare_for_decoder(self, srcs, masks):
        src_flatten = []
        mask_flatten = []
        spatial_shapes = []
        ...
        return src_flatten, mask_flatten, spatial_shapes, level_start_index, valid_ratios

    

    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None, img_ids=None):
        """
        Input:
        ...
        for i in range(len(srcs)):
            srcs[i] = self.se_layer(srcs[i])
        # prepare input for encoder

        src_flatten, mask_flatten, lvl_pos_embed_flatten, spatial_shapes, \
            level_start_index, valid_ratios = self.prepare_for_encoder(srcs[-1:], masks[-1:], pos_embeds[-1:])
        # two stage
        ...
        #memory to bs,h,w,c
        memory = memory.transpose(0, 1).reshape(srcs[-1].shape[0], self.d_model, spatial_shapes[0][0], spatial_shapes[0][1])
        srcs[-1] = memory


        # prepare input for decoder
        memory, mask_flatten, spatial_shapes, level_start_index, valid_ratios = self.prepare_for_decoder(srcs, masks)
        ...
        if self.two_stage_type =='standard':
            ...
            if self.embed_init_tgt:
                tgt_ = self.tgt_embed.weight[:, None, :].repeat(1, output_memory.shape[0], 1).transpose(0, 1) # nq, bs, d_model
            else:
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, 
        ...
        self.mask_feat = nn.Conv2d(1, d_model, 3, 1, 1)
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, 
        ...
        self.coco = COCO(annotation_file=coco_path+'/annotations/instances_train2017.json')
    ...
    def forward_prediction_heads(self, output, mask_feat, attn_mask_target_size):
        decoder_output = self.decoder_norm(output)
        decoder_output = decoder_output.transpose(0, 1)
        mask_embed = self.mask_embed(decoder_output)
        outputs_mask = torch.einsum("bqc,bchw->bqhw", mask_embed, mask_feat)
